...
import com.lingdu.base.fragment.MvvmFragment;
import com.lingdu.home.R;
import com.lingdu.home.databinding.FragmentHomeBinding;
...
public class HomeFragment extends MvvmFragment<FragmentHomeBinding, HeadlineNewsViewModel> implements HeadlineNewsViewModel.IMainView{
    //不同item必须不同的viewtype
...
//初始化
        ...
        viewDataBinding.mRecyclerView.setLayoutManager(layoutManager);
        //设置回收复用池大小，（如果一屏内相同类型的 View 个数比较多，需要设置一个合适的大小，防止来回滚动时重新创建 View）：
        RecyclerView.RecycledViewPool viewPool = new RecyclerView.RecycledViewPool();
        viewDataBinding.mRecyclerView.setRecycledViewPool(viewPool);
        viewPool.setMaxRecycledViews(0, 10);
        DelegateAdapter delegateAdapter = new DelegateAdapter(layoutManager, true);
        viewDataBinding.mRecyclerView.setAdapter(delegateAdapter);

        //banner
        BaseDelegateAdapter bannerAdapter = new BaseDelegateAdapter(getContext(), new LinearLayoutHelper()
            ...
            public void onBindViewHolder(BaseViewHolder holder, int position) {
                ...
                mBanner.setOnBannerListener(new OnBannerListener() {
                    ...
                    public void OnBannerClick(int position) {
                        Toast.makeText(getContext().getApplicationContext(), "banner点击了" + position, Toast.LENGTH_SHORT).show();
                    }
                });
        ...
        BaseDelegateAdapter menuAdapter = new BaseDelegateAdapter(getContext(), gridLayoutHelper, R.layout.vlayout_menu
                , 10, MENU_VIEW_TYPE) {
            @Override
            public void onBindViewHolder(BaseViewHolder holder, final int position) {
                ...
                holder.getView(R.id.ll_menu_home).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        Toast.makeText(getContext(), ITEM_NAMES[position], Toast.LENGTH_SHORT).show();
                    }
                });
            }
        };
        ...
        //news
        BaseDelegateAdapter newsAdapter = new BaseDelegateAdapter(getContext(), new LinearLayoutHelper()
        ...
        for (int i = 0; i < ITEM_URL.length; i++) {
            ...
            //item1 title
            final int finalI = i;
            BaseDelegateAdapter titleAdapter = new BaseDelegateAdapter(getContext(), new LinearLayoutHelper(), R.layout.vlayout_title, 1, TITLE_VIEW_TYPE) {
                @Override
                public void onBindViewHolder(BaseViewHolder holder, int position) {
                    super.onBindViewHolder(holder, position);
            ...
            BaseDelegateAdapter girdAdapter = new BaseDelegateAdapter(getContext(), gridLayoutHelper1, R.layout.vlayout_grid
                    , 4, GRID_VIEW_TYPE) {
                @Override
                public void onBindViewHolder(BaseViewHolder holder, final int position) {
                    super.onBindViewHolder(holder, position);
    ...
    @Override
    public void onChannelsLoaded(ArrayList<String> channels) {
